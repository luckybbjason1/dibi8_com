...
Prometheus는 만능 모니터링 솔루션이 아니다. 다음 제약 사항에 유의하라: 1. **네이티브 로그 집계 없음**: Prometheus는 지표를 처리하지만 로그는 처리하지 않는다. 로그 관리에는 Loki, ELK 또는 Fluentd가 필요하다.
