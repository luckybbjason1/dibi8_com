...
faq: - q: "커스텀 에이전트 정의 파일은 어디에 두며, 어떤 형식인가요?"
    a: "커스텀 에이전트는 YAML frontmatter가 붙은 Markdown 파일로, 프로젝트의 .claude/agents/ 디렉터리(또는 모든 프로젝트에서 쓰고 싶다면 ~/.claude/agents/)에 저장합니다. 파일명에서 .md 확장자를 뗀 것이 에이전트의 정체성은 아닙니다 — frontmatter의 name 필드가 정체성입니다. frontmatter는 name, description, 선택적 tools 화이트리스트, 선택적 model을 선언하며, 닫는 ---
 아래의 모든 내용이 에이전트의 시스템 프롬프트입니다."
