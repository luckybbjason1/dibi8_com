...
Traditional SCA tools (Snyk, Dependabot) check what your code declares as a dependency. They miss: - Globally installed CLIs (``npm install -g``, ``pip install --user``)
...
This is the feature most relevant to AI developers in 2026. Bumblebee scans the following config file paths: | File | Tool |
|
---
|
* * *
...
Bumblebee outputs **NDJSON** (newline-delimited JSON), one record per scanned artifact. This makes it easy to pipe into ``jq``, ingest into a SIEM, or diff against a previous inventory to detect new packages: `````bash
...
Bumblebee 2026: Perplexity AI Open-Sources Its Internal Supply-Chain Scanner for Developer Machines represents an important step forward in AI-powered development. As the ecosystem matures, we expect to see even more powerful capabilities emerge.
